...
public class HelperFunctionsRow extends HelperFunctions {
	...
	public void semiNaiveRecursion(Optimizations opt, CRV crvObj,
		...
		if (selectQuery.hasGroupBy) {
			...
			createTableR1 = "CREATE TABLE RG1 AS SELECT " +distinct+" 1 AS d, i AS i, j AS j, p AS p, v AS v"
					+ selectionCondition + " FROM " + initialTable;			
		...
		String createQuery = "CREATE TABLE "+crvObj.recursiveTable+" AS SELECT " + distinct + " * FROM RG1";
		for(int i=2;i<d;i++) {
