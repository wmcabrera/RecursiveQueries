insert into colrq_timetrack values('treeclique1m6_distopt','y','n','n','n',6,'n',TIMESTAMP 'now')
...
CREATE TABLE R5 AS SELECT DISTINCT 5 AS d, R4.i AS i, E.j AS j, R4.p*E.p AS p,R4.v+E.v AS v  FROM R4 JOIN E ON R4.j=E.i WHERE R4.i!=R4.j

SELECT COUNT(*) FROM R5

DROP table IF EXISTS R6 CASCADE

CREATE TABLE R6 AS SELECT DISTINCT 6 AS d, R5.i AS i, E.j AS j, R5.p*E.p AS p,R5.v+E.v AS v  FROM R5 JOIN E ON R5.j=E.i WHERE R5.i!=R5.j

SELECT COUNT(*) FROM R6

DROP table IF EXISTS reqa CASCADE

CREATE TABLE reqa AS SELECT DISTINCT   * FROM R1 UNION ALL SELECT * FROM R2 UNION ALL SELECT * FROM R3 UNION ALL SELECT * FROM R4 UNION ALL SELECT * FROM R5 UNION ALL SELECT * FROM R6
